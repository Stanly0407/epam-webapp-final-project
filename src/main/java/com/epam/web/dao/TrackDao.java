...
public class TrackDao extends AbstractDao<TrackDto> implements Dao<TrackDto> {
    ...
    private static final String FIND_TRACKS_BY_TITLE = "SELECT t.id, t.release_date, t.title, t.description, t.price, t.filename, a.id, a.name, a.lastname " +
            "FROM track t INNER JOIN artist a ON t.artist_id=a.id WHERE t.title=?";
    private static final String FIND_TRACKS_BY_ARTIST_NAME = "SELECT t.id, t.release_date, t.title, t.description, t.price, t.filename, a.id, a.name, a.lastname " +
            "FROM track t INNER JOIN artist a ON t.artist_id=a.id WHERE a.=?";
    private static final String FIND_TRACKS_BY_ALBUM_TITLE = "SELECT t.id, t.release_date, t.title, t.description, t.price, t.filename, a.id, a.name, a.lastname " +
            "FROM track t INNER JOIN artist a ON t.artist_id=a.id WHERE t.title=?";
    private static final String FIND_TRACKS_BY_COLLECTION_TITLE = "SELECT t.id, t.release_date, t.title, t.description, t.price, t.filename, a.id, a.name, a.lastname " +
            "FROM track t INNER JOIN artist a ON t.artist_id=a.id WHERE t.title=?";
    ...
    public void findMusicByCondition(String searchSubject, String searchCondition) throws DaoException {
        ...
        executeUpdate(query, new TrackDtoRowMapper(), searchSubject);
    }
